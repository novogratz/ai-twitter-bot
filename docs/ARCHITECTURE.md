...
followed accounts and the handles the run tried, whatever the case;
...
and returns `None`, and the job goes on. The caps, the order and the
persistent memory of tried handles stay in the job. `follow_engagers_job`
