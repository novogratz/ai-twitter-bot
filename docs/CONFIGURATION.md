...
| `LLM_FALLBACK_CLI` | Unset: no fallback, a failed call fails, Originals and Replies alike. `codex` (or `gemini`, `ollama`) opts into one; an unknown name fails the fallback without running anything |
...
| Variable | Default | Purpose |
|---|---|---|
| `AI_CLI` | `ollama` | `ollama` / `codex` / `opencode` / `gemini`. `ollama` uses the direct local HTTP path. An unknown name fails the call. |
| `LLM_FALLBACK_CLI` | (unset) | Fallback provider used when the primary LLM fails, times out, is missing, or returns empty output. Unset or empty: no fallback. |
