...
|---|---|---|
| `DEBATE_MAX_TURNS_PER_AUTHOR_PER_DAY`, Debate turns per Engager per day | at most 4 | 4 |
| `MIN_SECONDS_BETWEEN_REPLIES`, gap between Replies | at least 8 s | 8 |
| `REPLY_JITTER_SECONDS`, Reply jitter | at least 0 | 7 |
| `LIKE_BOT_PER_CYCLE`, likes per `like_job` cycle | at most 10 | 10 |
| `LIKE_BOT_DAILY_CAP`, `like_job` likes per day | at most 500 | 500 |
| `FOLLOW_TOTAL_CAP`, accounts followed in total | at most 3500 | 300 |
| `MAX_FOLLOWS_PER_DAY`, follows per day | at most 20 | 20 |
| `FOLLOWBACK_CAP`, Follow-backs per cycle | at most 8 | 8 |
| `FOLLOW_ENGAGERS_PER_DAY`, Engagers followed per day | at most 10 | 10 |
| `FOLLOW_ENGAGERS_PER_CYCLE`, Engagers followed per cycle | at most 2 | 2 |
...
| `DUP_SHARED_BIGRAMS`, shared bigrams that make a duplicate | at most 3 | 3 |
| `DUP_TOPIC_SHARED_WORDS`, shared words that make a same story | at most 3 | 3 |
...
a longer window, catches more duplicates. The older bounds hold as before:
`MAX_ORIGINALS_PER_DAY` at most 8, `MIN_SECONDS_BETWEEN_POSTS` at least
