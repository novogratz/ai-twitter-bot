...
@pytest.mark.parametrize("primitive, bound", [
    ("open_url", "OPEN_TIMEOUT_S"),
    ("close_front_tab", "CLOSE_TIMEOUT_S"),
    ("_scroll_page", "SCROLL_TIMEOUT_S"),
])
def test_a_wedged_osascript_gives_the_safari_lock_back(monkeypatch, unwalled, primitive, bound):
    """#251: open_url, the tab close and the scroll had no timeout, so a
    wedged osascript held the Safari lock for good. Past its bound the
    child is killed, the primitive returns and the lock is free. A `sleep`
    child stands in for the wedged osascript."""
    import subprocess
    import threading
    import time
    from src.x import safari
    ...
    args = ("https://x.com/home",) if primitive == "open_url" else ()

    started = time.monotonic()
    with safari._safari_lock:
        result = getattr(safari, primitive)(*args)
    assert time.monotonic() - started < 5
    assert result is (False if primitive == "open_url" else None)
