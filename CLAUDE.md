...
- **`src/early_bird_bot.py`** - Early-bird reply path. ~5 min jittered (12-min freshness window), scans 3 of ~125 mega accounts (FR/QC/EN AI+crypto+bourse, expanded 2026-05-06 with naval, paulg, balajis, pmarca, swyx, soumithchintala, FR macro/banque centrale, etc.). Cap 4 replies per cycle. Top-5-reply is 10-100x impressions vs. late-reply.
- **`src/retweet_bot.py`** - Selective retweet path. Every 20 min, cap 60/day (2026-05-06). FR/EN split: 26 FR press handles + 25 EN wires; per-cycle sample 14 FR + 4 EN. Scorer adds +2 for FR-language signal (accents, FR words) and -1 to EN picks → EN needs score ≥8 to retweet vs ≥7 for FR. Threshold 6 (relaxed for volume). Picks ≥6 → retweet, picks ≥7 → also appended to `daily_news_picks.md`. Persistent dedup `retweeted.json`.
- **`src/thread_bot.py`** - 1 well-crafted FR thread per day on the biggest IA/crypto/bourse story. 4 tweets: hook → fait → angle → chute + URL. Idempotent via `thread_daily_state.json`; scheduler fires every 4h to catch up after restart. Different distribution surface than single news (longer lifespan, screenshot RTs).
- **`src/promote_bot.py`** - Quote-RTs our highest-engagement reply onto the profile feed. Replies live inside other people's threads (invisible to followers); promote re-surfaces them on /kzer_ai. Cap 3/day, MIN_LIKES=5, dedup via `promoted_replies.json`. Pulls candidates by scraping /with_replies. Every 3h.
- **`src/followback_bot.py`** - Scrapes /kzer_ai/followers via JS, follows back fresh accounts (cap 8/cycle, every 2h). Bot-handle filter rejects no-vowel + mostly-digit + BLOCKLIST handles. Persists via `engage_bot.followed_accounts.json` so no double-follow with discover/scout.
- **`src/pin_bot.py`** - Daily idempotent. Picks our highest-engagement own POST (not reply) from the recent profile window and pins it via JS menu click on the X tweet-action menu. MIN_LIKES_TO_PIN=5 (better empty than stale). State in `pin_daily_state.json` + `pin_history.json`.
- **`src/like_bot.py`** - Bulk-like FR niche tweets. Every 15 min, opens a search query (rotating FR pool), JS-clicks ~18 visible like buttons. ~70 notifications/hour outbound — strangers get pinged → click through to profile.
- **`src/viral_followup_bot.py`** - When own post hits ≥ VIRAL_THRESHOLD likes (default 15), generates a short FR follow-up reply that extends the punchline + posts in-thread. Capitalize on the algorithm push window (first ~hour after takeoff). Persisted dedup via `viral_followed_up.json`. Cap 3/cycle every 30min.
- **`src/digest_thread_bot.py`** - Daily 6-tweet recap thread covering the 5 biggest IA/crypto/bourse stories of the last 36h. Different format than `thread_bot.py` (single-story dissect): bundle for high shareability. Idempotent via `digest_thread_state.json`. Different from `thread_bot.py` so both run daily.
