...
The bot runs one tweet immediately on start, then schedules the next run every 30 minutes (hardcoded in `random_interval_minutes()`). Each tick calls `safe_run_bot_cycle()`, which catches all exceptions so the scheduler stays alive.
