...
- What describes the Account, not the engine (its handle, Slots, feeds, trusted hosts, relevance filter, network, niche, X searches, Relations), goes in `accounts/<name>/account.toml`, checked by `src/core/account.py`, and is read with `account.current()` inside the function; its Voice (`voice_fr.md`, `voice_en.md`) and its Relations' prompts (`relations/`) sit next to it. The engine names no account, in code or in a comment that is not a dated record: say "the Account's profile" or "a Relation" (`tests/test_engine_names_no_account.py`).
