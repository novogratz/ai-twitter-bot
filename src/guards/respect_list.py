...
The list is the Operator's file accounts/<BOT_ACCOUNT>/respect_list.json,
edited by hand and versioned; the bot never writes it. While it is missing
or unreadable, every function that reads it raises StateUnreadable,
render_block included, and nothing recreates it.
"""
import re
...
# The Operator's list, in the Account folder: the bot reads it, never writes it.
RESPECT = OperatorFile("respect_list.json", dict)


def load() -> set:
    """Return the current respect list as a set of lowercased handles."""
    return set(RESPECT.read().get("handles", {}).keys())
