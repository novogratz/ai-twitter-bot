...
| **Amplification** | `retweet_bot`, `quote_tweet_bot`, `hot_quote_bot`, `feed_sweeper_bot`, `boost_recycler_bot` | QRT SURGE (≤100/day, 50-like floor) — number-reframe + metaphor + closing question on mid-size finance/AI posts; AI-vs-Bitcoin feud bit; own winners recycled via un-RT→re-RT (4h gaps, max 4); plain RTs 0-2/day |
| **Replies** | `direct_reply`, `reply_bot`, `engagement_targeting`, `early_bird_bot`, `mega_watch_bot`, `replyback_agent`, `btc_blitz` | The core engine — unlimited throughput, freshest-first; scan pools self-curated by `account_curator` (pinned: TheBTCTherapist + Graphseo); bestie blitz covers every ≤48h BTCTherapist post |
