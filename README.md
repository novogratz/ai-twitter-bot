...
| **Replies** | `direct_reply`, `reply_bot` (LLM-search, off by default since 2026-07-19 — `ENABLE_REPLY_SEARCH`), `engagement_targeting`, `early_bird_bot`, `mega_watch_bot`, `replyback_agent`, `debate_bot` (mentions-driven debates, 2026-07-19), `follow_engagers_bot` (follow-back farming from repliers), `self_quote_bot` (1/day self-QRT of a winner), `btc_blitz` | The core engine — unlimited throughput, freshest-first; discovery is home/search-only since 2026-06-07 (profile visits gated to own profile + `PROFILE_VISIT_ALLOWLIST`, default TheBTCTherapist); bestie blitz covers every ≤48h BTCTherapist post |
