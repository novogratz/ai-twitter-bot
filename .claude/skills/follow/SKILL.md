...
2. Preconditions in `docs/OPERATIONS.md#manual-writes`. Check:
   ...
   prints `True`.
3. Run `uv run python -c "from src.guards import follow_policy as p; from src.x.twitter_client import follow_account; print(follow_account('$username', relations=p.SEED_ONLY))"`
   - A manual follow is for a Seed account (whitelist) only, as before
     #173: the command asks the follow policy for Seed accounts only, and
     the policy finds the relation itself. A follower or an Engager comes
     back `REFUSED` with nothing opened, logged as `outside the relations
     asked (Seed account)`. Follow-backs and Engager follows belong to their
     jobs.
