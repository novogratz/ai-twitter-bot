...
1. Running? `pgrep -if "python.*main\.py"` and `cat bot.lock` (PID of the
   running `main.py`).
2. Toronto time: `TZ=America/Toronto date "+%F %T %Z"`. Active hours are
   04:30–22:00; silence outside them is normal.
3. Today's profile posts against the ceiling of seven:
...
Report:
- Bot running? (yes/no + PID), awake or asleep
- Today: profile posts X/7, slots published / pending
