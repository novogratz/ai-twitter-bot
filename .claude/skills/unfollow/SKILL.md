...
1. Check the bot is stopped (`pgrep -if "python.*main\.py"`) — it shares Safari.
   If running, suggest `/stop` first (the script also refuses on its own).
   Run it during active hours only (04:30–22:00 Toronto): the script drives
   `osascript` directly and does not check them.
...
4. Keep-set (`--keep`): default `whitelist` = current `whitelist.json` tiers +
   `seeds[]` (the curated follow list — full purge; never unfollow what the
   follow policy may follow, or the 30d anti-churn record would block the
   re-follow). `--keep legacy` adds respect_list +
...
5. Report the final `TOTAL unfollowed:` count and the new ratio if available.
   ...
   Pace: `--pace normal|fast|brisk|insane` — default `normal` (3.5-7s gaps,
   operator-preferred). Every pace auto-recovers: on a blue rate-limit toast
   (`[data-testid=toast]`) or 5 failed confirms it cools down
   `--cooldown-mins` (default 2, +50% per consecutive hit, cap 4x), clears
   stale cell tags, and resumes until the list is empty — never aborts.
   Faster paces don't gain much: X drains its ~190/window unfollow quota
   regardless, then it's all cooldowns.
